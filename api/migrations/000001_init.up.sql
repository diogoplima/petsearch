-- 000001_init.up.sql — Pet Search schema (self-hosted; authorization lives in
...
create table reports (
  ...
  -- null = anonymous sighting
  status report_status not null,
  state report_state not null default 'open',
  species text not null default 'dog',
  -- launch is dogs-only; column is the expansion hedge (ADR-0012)
  dog_name text,
  breed text,
  ...
  -- bumped by owner activity / "still searching" (ADR-0014)
  embedding vector(512),
  -- dormant until v2 (ADR-0005)
  created_at timestamptz not null default now(),
  resolved_at timestamptz,
  constraint missing_requires_owner check (
    status = 'sighted'
...
create table report_private (
  ...
  contact_info text,
  -- owner-provided; revealed only on confirmed match
...
create function fuzz_location() returns trigger language plpgsql as $$
declare seed double precision;
begin seed := (hashtext(new.report_id::text)::bigint % 100000) / 100000.0;
update reports
...
insert
  or
update of exact_location on report_private for each row execute function fuzz_location();
-- ---------------------------------------------------------------------------
-- Matches
-- ---------------------------------------------------------------------------
create table matches (
  ...
  suggested_by uuid references users (id) on delete
  set null,
...
) returns table (report_id uuid, distance_km numeric, score real) language plpgsql stable as $$
declare src reports;
src_loc geography;
want report_status;
begin
...
if not found then raise exception 'report not found';
end if;
select exact_location into src_loc
from report_private rp
...
if want is null then raise exception 'matching not defined for status %',
src.status;
end if;
return query
select r.id,
  round(
    (st_distance(rp.exact_location, src_loc) / 1000.0)::numeric,
    1
  ),
  (
    (
      1.0 - st_distance(rp.exact_location, src_loc) / p_radius_m
    ) * 0.5 + case
      when r.breed is not null
      and r.breed = src.breed then 0.2
      else 0
    end + case
      when r.size is not null
      and r.size = src.size then 0.1
      else 0
    end + (
      cardinality(
        array(
          select unnest(r.colors)
          intersect
          select unnest(src.colors)
        )
      )::real * 0.1
    ) + greatest(
      0,
